<?php
// production_frontend_consumer.php
...
// Declare the production frontend queue
$queueName = 'production_frontend_queue';
$channel->queue_declare($queueName, false, true, false, false);

echo " [*] Waiting for packages on {$queueName}. To exit press CTRL+C\n";

$callback = function ($msg) {
    echo " [x] Received package for Production Frontend\n";
    ...
    $packageDir = "/tmp/production_frontend_{$packageName}_v{$version}";
    ...
    if ($zip->open($packagePath) === TRUE) {
        ...
        if (file_exists($setupScript)) {
            ...
            if ($return_var === 0) {
                echo " [x] Setup script executed successfully for Production Frontend\n";
            } else {
                echo " [!] Setup script execution failed for Production Frontend with exit code {$return_var}\n";
                echo " [!] Output:\n";
                echo implode("\n", $output) . "\n";
            }
        } else {
